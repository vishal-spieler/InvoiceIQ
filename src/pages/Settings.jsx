      ...
      <div className="grid gap-6 h-full" style={{ gridTemplateColumns: '240px 1fr', alignItems: 'start' }}>
        ...
        <div className="card" style={{ padding: '8px' }}>
          <ul className="col gap-1">
            {['Extraction Engine', 'Feature Flags', 'Database Connection', 'Notifications', 'API Keys', 'User Management'].map((item) => {
          ...
          {activeTab === 'Feature Flags' && (
            <>
              <div>
                ...
                <p className="text-t2 text-xs">Control which modules are visible in the sidebar.</p>
              </div>

              <div className="col" style={{ gap: '0' }}>
                {[
                  { label: 'Inbox Monitor', sub: 'View and manage incoming email queue', key: 'inboxMonitor' },
                  { label: 'Processing Queue', sub: 'Live extraction job status', key: 'processingQueue' },
                  { label: 'Email Reports', sub: 'History of success/failure reply emails', key: 'emailReports' },
                  { label: 'Resend Failures', sub: 'Retry failed invoice extractions', key: 'resendFailures' },
                  { label: 'Flow Diagram', sub: 'Visual email ingestion pipeline', key: 'flowDiagram' },
                  { label: 'Email Config', sub: 'IMAP, SMTP, and processing rules', key: 'emailConfig' },
                  { label: 'Reply Templates', sub: 'Customise vendor reply emails', key: 'replyTemplates' },
                ].map((flag) => {
                  const isOn = flags[flag.key];
                  return (
                    <div key={flag.key} className="flex justify-between items-center" style={{ padding: '12px 0', borderBottom: '1px solid var(--b)' }}>
                      <div>
                        <div style={{ fontWeight: 500, fontSize: '13px', color: 'var(--t)' }}>{flag.label}</div>
                        <div style={{ fontSize: '11px', color: 'var(--t2)' }}>{flag.sub}</div>
                      </div>
                      <div className={`toggle ${isOn ? 'on' : ''}`} onClick={() => {
                        toggleFlag(flag.key);
                        if (!isOn) toast(`✅ ${flag.label} enabled`, 'green');
                        else toast(`⚠️ ${flag.label} has been disabled`, 'amber');
                      }}>
